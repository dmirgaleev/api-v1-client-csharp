...
namespace Info.Blockchain.API.Client
{
	public class BlockchainHttpClient : IHttpClient
	{
		...
		public async Task<long> GetLongAsync(string route)
		{
			if (route == null)
			{
				throw new ArgumentNullException(nameof(route));
			...
			if (!long.TryParse(responseString, out var result))
			{
		...
		public async Task<int> GetIntAsync(string route)
		{
			if (route == null)
			{
				throw new ArgumentNullException(nameof(route));
			...
			HttpResponseMessage response = await httpClient.GetAsync(route);
			var responseString = await ValidateResponse(response);
			if (!int.TryParse(responseString, out var result))
			{
				throw new InvalidCastException();
			}
			return result;
